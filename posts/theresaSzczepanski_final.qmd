...
Item_Type_Points <-function(subject, itemType, subjectItemDF){
    if(subject == "science"){
          subjectItemDF%>%
...
##To-Do update points available for math and ELA and test...    
    }
  else if (subject == "math"){
    subjectItemDF%>%
...
Practice_Cat_Points <-function(subject, practiceCategory, subjectItemDF){
    if(subject == "science"){
          subjectItemDF%>%
          select(`sitem`, `item Possible Points`, `Science Practice Category`)%>%
          group_by(`Science Practice Category`)%>%
          summarize(available_points = sum(`item Possible Points`, na.rm=TRUE))%>%
          filter(`Science Practice Category` == practiceCategory)#%>%
...
#### Test Data Frames------------------------------------------------------------------
G9ScitestDF<-read_item("_data/2022MCASItemAnalysis.xlsx", "SG9Physics", "science")
testDFELA<-read_item("_data/2022MCASItemAnalysis.xlsx", "EG10", "ela", 10)
#view(testDF)
# testDF%>%
#   select(`sitem`, `item Possible Points`, `Reporting Category`)%>%
#   group_by(`Reporting Category`)%>%
...
testDF
MDataPoints<-Practice_Cat_Points("science", "Mathematics and Data", testDF)
MDataPoints
ERMPoints<-Practice_Cat_Points("science", "Evidence, Reasoning, and Modeling", testDF)
ERMPoints
IQPoints<-Practice_Cat_Points("science", "Investigations and Questioning", testDF)
IQPoints
ENPoints<-Reporting_Cat_Points("science", "EN", testDF)
ENPoints
MFPoints<-Reporting_Cat_Points("science", "MF", testDF )
MFPoints
WAPoints<-Reporting_Cat_Points("science", "WA", testDF )
WAPoints
CRPoints<-Item_Type_Points("science", "CR", testDF)
CRPoints
SRPoints<-Item_Type_Points("science", "SR", testDF)
SRPoints
#----------------------------------ELA function tests -------------------------------------------
testDFELA
